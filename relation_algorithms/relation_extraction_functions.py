...
class LLM_Relation_Extractor:
    ...
    def identify_chapters(self):
        ...
        return [chapter for chapter in chapters.split('\n')][2:]
    ...
    def identify_learning_concepts_outcomes(self, chapter_dict: dict) -> list:
        return_list = []
        current_concept = ''

        for chapter, chapter_name in chapter_dict.items():
            current_concept = llm(f"Please identify the main learning outcomes and main learning concepts given {chapter}, the chapter name is {chapter_name}. Here is the textbook in which to retrieve them: {self.link}")
    ...
    def association_identification(self, concept_list: list) -> dict:
    ...
    def dependency_relation_extraction(self, keys: list, relations_dict = None) -> dict:
        relation = ''
    ...
    def print_interactive_graph(self) -> None:
